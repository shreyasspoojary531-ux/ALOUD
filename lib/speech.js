 ...
 * - Clean timer cancellation via activeRepeatTimeout
 */

let cachedVoices = [];
let activeUtterance = null; // Global V8 GC reference retention
let activeRepeatTimeout = null; // Global inter-repeat pause timer retention
...
export function cancelSpeech() {
  if (activeRepeatTimeout) {
    clearTimeout(activeRepeatTimeout);
    activeRepeatTimeout = null;
  }
...
export function say(text, callbacks = {}) {
  ...
  // Cancel any ongoing speech or pending repeat timers before starting fresh message
  cancelSpeech();

  const isLoopMode = callbacks.repeat === "loop";
  const repeatTotal = isLoopMode ? Infinity : Math.max(1, callbacks.repeat ?? 1);
  let repeatsDone = 0;
  ...
  const playRepeat = () => {
    ...
    const playChunk = (index) => {
      if (index >= chunks.length) {
        ...
        repeatsDone += 1;
        if (isLoopMode || repeatsDone < repeatTotal) {
          // Pause 800ms then play next repeat
          activeRepeatTimeout = setTimeout(playRepeat, 800);
        } else {
          activeUtterance = null;
          activeRepeatTimeout = null;
          callbacks.onEnd?.();
